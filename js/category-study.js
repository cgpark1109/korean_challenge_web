...
const CategoryStudy = (() => {
  ...
  function sortWordsByHangul(items) {
    return [...items].sort((a, b) => koCollator.compare(a.char, b.char));
  }
  ...
  function filterWords(items, query) {
    ...
    return items.filter(
      (item) =>
        item.char.includes(trimmed) ||
        item.roman.toLowerCase().includes(q) ||
        (item.meaning || '').toLowerCase().includes(q),
    );
  }

  function buildWordToolbar(onSearch) {
    const toolbar = document.createElement('div');
    toolbar.className = 'category-page__word-toolbar';

    const input = document.createElement('input');
    input.type = 'search';
    input.className = 'category-page__word-search';
    input.placeholder = 'Search Korean, roman, or meaning';
    input.setAttribute('aria-label', 'Search words');
    input.addEventListener('input', () => onSearch(input.value));

    const count = document.createElement('span');
    count.className = 'category-page__word-count';
    ...
    return {
      ...
      updateCount(shown, total) {
        count.textContent = `${shown.toLocaleString()} / ${total.toLocaleString()}`;
      },
    };
  }
  ...
  function renderWords(container, items, meta = {}) {
    container.innerHTML = '';

    if (meta.updateCount) {
      meta.updateCount(items.length, meta.total ?? items.length);
    }

    if (items.length === 0) {
      const empty = document.createElement('p');
      empty.className = 'category-page__word-empty';
      empty.textContent = meta.query?.trim() ? 'No words found' : 'No words';
  ...
  async function init({ title, fromPage, mode, getItems, getGroups }) {
    ...
    if (mode === 'words') {
      ...
      const sortedItems = sortWordsByHangul(rawItems);

      let wordToolbar;
      const renderWithFilter = (query) => {
        const filtered = filterWords(sortedItems, query);
        renderWords(container, filtered, {
          query,
          total: sortedItems.length,
          updateCount: wordToolbar?.updateCount,
        });
      };

      wordToolbar = buildWordToolbar(renderWithFilter);
      container.parentNode.insertBefore(wordToolbar.element, container);
      renderWithFilter('');
